<template>
  ...
  <el-form ref="form" :model="form" label-width="80px">
    ...
    <el-form-item v-if="childrenlist" label="子分类">
      <el-select v-model="form.t_id" placeholder="请选择">
        <el-option v-for="item in childrenlist" :key="item.id" :label="item.name" :value="item.id"></el-option>
      </el-select>
    </el-form-item>
...
export default {
  ...
  methods: {
    async getsons () {
      const data = await getson({id:this.form.parent})
      console.log('1111111111')
      this.childrenlist = data.arr
