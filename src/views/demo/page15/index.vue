<template>
  <d2-container>
    ...
    >
      ...
      <el-table-column align="center" prop="created_at" label="是否置顶" width>
        <template slot-scope="scope">
          <el-tag effect="dark" type="success" v-if="scope.row.top === '1'">置顶中</el-tag>
          <el-tag type="danger" v-else>未置顶</el-tag>
        </template>
      </el-table-column>
...
<script>
import { getchannellist, getchanneldel } from "../../../api/http";
...
export default {
  ...
  methods: {
    async getbanners(page) {
      const data = await getchannellist({ page });
    ...
    async getdele() {
      ...
      const data = await getchanneldel(id);
      if (data.code == 200) {
    ...
    getadds() {
      ...
      this.$router.push({ name: "pageadd6" });
    },
    // 修改
    gettck(data) {
      //   this.dialogFormVisible = true;
