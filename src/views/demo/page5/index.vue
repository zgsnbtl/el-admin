<template>
  <d2-container>
    ...
    >
      ...
      <el-table-column align="center" width label="标题">
...
export default {
  ...
  methods: {
    ...
    async getdele() {
      const data = await getnewsdelete({ id: this.deleteId });
      if (data.code === 200) {
    ...
    this.$router.push({name:'pageadd2'})
    },
    // 修改
    gettck(data) {
    //   this.dialogFormVisible = true;
    
    this.$router.push({path:'/pageadd2',query: {
          id: data.id, code: '修改'
    }})
    },
