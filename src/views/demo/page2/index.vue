<template>
  <d2-container>
    ...
    >
      ...
      <el-table-column width="" label="店铺链接">
        <template slot-scope="scope">
          {{ scope.row.link }}
        </template>
      </el-table-column>
      <el-table-column label="店铺地区" width="" align="center">
        <template slot-scope="scope">
...
<script>
import {getdiagnose,getdpdelete} from '../../../api/http'
export default {
  name: 'page2',
  data() {
    return {
  ...
  methods: {
    // 店铺诊断接口
    async getdge(page) {
      const data = await getdiagnose({page})
    ...
    async getdele () {
      const data = await getdpdelete({id: this.deleteId})
      if (data.code === 200) {
        this.getdge(this.page)
