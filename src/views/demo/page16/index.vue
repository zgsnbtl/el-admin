<template>
  <d2-container>
    ...
    >
      ...
      <el-table-column width="" label="问题">
        <template slot-scope="scope">
          {{ scope.row.problem }}
        </template>
      </el-table-column>
...
<script>
import {getproblem,getproblemdel} from '../../../api/http'
export default {
  name: 'page2',
  data() {
    return {
  ...
  methods: {
    // 店铺诊断接口
    async getdge(page) {
      const data = await getproblem({page})
    ...
    async getdele () {
      const data = await getproblemdel({id: this.deleteId})
      if (data.code === 200) {
        this.getdge(this.page)
