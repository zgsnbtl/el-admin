<template>
  <d2-container>
    <!-- <template slot="header">店铺诊断展示</template> -->
    <el-table
    ...
    >
      ...
      <el-table-column width="" label="报名名称">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
      <el-table-column label="报名课程" width="" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" label="手机号" width="" align="center">
        <template slot-scope="scope">
...
<script>
import {getmakelist,getmakedelete} from '../../../api/http'
export default {
  name: 'page7',
  data() {
    return {
  ...
  methods: {
    // 店铺诊断接口
    async getdge(page) {
      const data = await getmakelist({page})
    ...
    async getdele () {
      const data = await getmakedelete({id: this.deleteId})
      if (data.code === 200) {
        this.getdge(this.page)
