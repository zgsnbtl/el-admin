...
<script>
import { getdynamicadd, getdynamicux, getdynamicup, getcllist} from "../../../api/http";
...
export default {
  ...
  methods: {
    ...
    async getcllists () {
      const data = await getcllist({status:2})
    ...
    async submit() {
      ...
      if (item.code === "xg") {
        const data = await getdynamicup(this.$refs.datali.form);
        if (data.code === 200) {
      ...
      const data = await getdynamicadd(this.$refs.datali.form);
      if (data.code === 200) {
        this.close({tagName:this.current});
        this.$message.success("提交成功！");
        this.$router.push({ name: "page4" });
    ...
    async getup() {
      let item = this.$route.query;
      if (item.code === "xg") {
        const data = await getdynamicux({ id: item.id });
